...
\hypersetup{
  ...
  pdfsubject={Toetsen maken met de TISD Exam Document Class},
...
\Option{dyslect} -- zet een aantal zaken goed voor dyslecten
...
\verb|\choicelabel| -- opnieuw gedefinieerd als \verb|\alph{choice})| (let op het haakje).
...
xcolor, babel, geometry, hyperref, amssymb

\medskip
In de `vanilla-mode'worden de volgende packages gebruikt:

xcolor , babel, geometry, amssymb
...
%%\item[\texttt{Class command `hulpmiddelen' is deprecated as of version 1.7}]
%%  Dit commando is verwijderd in versie 1.7.
\end{description}
